...
- The Linux agent VM is `homeConfigurations.agent-vm` (Graviton: `agent-vm-aarch64`). Do not commit Pi auth, Claude credentials, sessions, or `~/.agents/skills` here. `sync-skills.sh` copies skills between machines. Claude `settings.json` stays per machine because it names trusted repos.
